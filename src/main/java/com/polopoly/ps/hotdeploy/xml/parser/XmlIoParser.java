...
class XmlIoParser extends AbstractParser {
        ...
        for (Element element : children(contentElement)) {
			...
			if (nodeName.equals("metadata")) {
            	...
            	if (nodeName.equals("component")) {
            		parsedContent.setComponent(element.getAttribute("group"), element.getAttribute("name"), element.getTextContent());
            	}
            		...
            		if (element.getAttribute("encoding").equals("relative")) {
            			...
            			// don't want to log this as an error since the caller might just want the ParseCallback.
            		}
            	}
    ...
    private void readFile(TextContent content, String storeInFile, String readFromFileName) {
    	...
    	try {
			...
			LOGGER.log(Level.WARNING, "Could not read file " + readFromFileName + " relative to "
					+ baseUrl + ".");
		} catch (IOException e) {
			LOGGER.log(Level.WARNING, "Could not read file " + readFromFileName + " relative to "
					+ baseUrl + ".");
