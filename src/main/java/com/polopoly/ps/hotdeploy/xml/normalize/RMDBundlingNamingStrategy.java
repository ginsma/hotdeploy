package com.polopoly.ps.hotdeploy.xml.normalize;

import static com.polopoly.ps.hotdeploy.client.Major.REFERENCE_METADATA;
 ...
 * between the objects.
 */
public class RMDBundlingNamingStrategy extends
		DefaultNormalizationNamingStrategy {
	private static final Logger LOGGER = Logger
			.getLogger(RMDBundlingNamingStrategy.class.getName());

	public RMDBundlingNamingStrategy(PolicyCMServer server,
	...
	public File getFileName(ContentRead content) {
		if (isReferenceMetadata(content)) {
			...
			try {
				...
				LOGGER.log(Level.WARNING, "Could not get security parent of "
						+ content.getContentId().getContentIdString()
						+ " with ID " + parentId.getContentIdString() + ": "
						+ e.getMessage(), e);
			}
		}
	...
	private boolean isReferenceMetadata(ContentRead content) {
		return content.getContentId().getMajor() == REFERENCE_METADATA
				.getIntegerMajor();
	}
}
