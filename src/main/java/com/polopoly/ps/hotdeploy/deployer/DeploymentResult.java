...
public class DeploymentResult {
	private static final Logger logger = Logger.getLogger(DeploymentResult.class.getName());
	...
	private void addMessage(Map<DeploymentFile, DeployResult> map, DeploymentFile file, String message) {
	...
	public int getTotalReportedCount() {
		return getSuccessfulFilesCount() + getFailedFilesCount() + getPartialFilesCount();
	}
	...
	public String getSuccessfulResultString() {
		...
		return count(cnt, "file") + " " + (cnt == 1 ? "was" : "were") + " successfully imported.";
	}
	...
	public String getPartialResultString() {
		...
		sb.append(count(getPartialFilesCount(), "file") + " "
				+ (getPartialFilesCount() == 1 ? "was" : "were") + " only partially imported:");
	...
	public void logResults() {
		if (filesToDeployCount > 0) {
			...
			logger.log(Level.INFO, count(unmodifiedFiles, "file") + " had not been modified and "
