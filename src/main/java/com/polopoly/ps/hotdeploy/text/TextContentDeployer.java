...
public class TextContentDeployer {
    private static final Logger logger = Logger.getLogger(TextContentDeployer.class.getName());
        ...
        try {
            ...
            for (TextContent textContent : contentSet) {
                try {
                    ...
                    throw new DeployException("While creating object to publish " + textContent.getId() + " in: " + e,
                        e);
                }
            }
            ...
            for (TextContent textContent : contentSet) {
                try {
                    for (Publishing publishing : textContent.getPublishings()) {
                        ...
                        publish(newVersion, publishInVersion, publishing.getPublishInGroup(), publishing.getPublishIn()
                            .resolveReference(server).getReferenceMetaDataId());
                    }
                } catch (CMException e) {
            ...
            for (TextContent textContent : contentSet) {
                ...
                try {
                    ...
                    throw new DeployCommitException("While committing " + toString(newVersion) + ": " + e, e);
                } catch (Exception e) {
                    throw new DeployCommitException("While committing " + toString(newVersion) + ": " + e, e);
                }
    ...
    private void publish(Policy publish, Policy publishIn, String publishInGroup, ContentId metadata)
        throws CMException {
    ...
    private boolean contains(ContentList contentList, ContentId contentId) throws CMException {
        for (int i = contentList.size() - 1; i >= 0; i--) {
            ContentId referredContentId = contentList.getEntry(i).getReferredContentId();

            if (referredContentId != null && referredContentId.equalsIgnoreVersion(contentId)) {
                return true;
            }
        }
    ...
    private void createPublishInVersion(TextContent textContent, Map<String, Policy> newVersionById) throws CMException {
        for (Publishing publishing : textContent.getPublishings()) {
            ...
            if (!newVersionById.containsKey(publishInExternalId)) {
                try {
                    Policy newPublishInVersion = createNewVersionOfExistingContent(publishIn.resolveId(server));
    ...
    private void deploy(TextContent textContent, Policy policy) throws DeployException, CMException {
        ...
        if (templateId != null) {
            ...
            if (template == null) {
                throw new DeployException("The object \"" + templateId + "\" specified as template of " + textContent
                                          + " must be defined in the same file.");
            }

            if (template.getTemplateId() != null) {
                throw new DeployException("Cannot use the object \"" + templateId + "\" as template of " + textContent
    ...
    private void setInputTemplate(TextContent textContent, Content content) throws CMException {
    ...
    private void setSecurityParent(TextContent textContent, Content content) throws CMException {
    ...
    private void setLists(TextContent textContent, Content content) throws CMException {
        for (Entry<String, List<Reference>> groupEntry : textContent.getLists().entrySet()) {
            ...
            for (Reference reference : groupEntry.getValue()) {
                ContentReference resolvedReference = reference.resolveReference(server);

                if (resolvedReference.getReferenceMetaDataId() != null) {
                    makeSureMetadataHasReference(resolvedReference.getReferenceMetaDataId(),
                ...
                try {
                    ...
                    if (!ignoreContentListAddFailures) {
                        ...
                        logger
                            .log(Level.WARNING, "Could not add " + reference + " to content list " + group + " in "
                                                + content.getContentId().getContentIdString() + ": " + e.getMessage());
                    }
                }
            }
        }
    ...
    private void makeSureMetadataHasReference(ContentId referenceMetaDataId, ContentId referredContentId) {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Could not read metadata content " + referenceMetaDataId.getContentIdString()
                                      + ".");
        ...
        try {
            currentReferredId = metadataContent.getContentReference("polopoly.ReferenceMetaData", "referredId");
        } catch (CMException e3) {
            logger
                .log(Level.WARNING,
                     "Could not read reference in metadata content " + referenceMetaDataId.getContentIdString() + ".");
        ...
        if (currentReferredId == null) {
            ...
            try {
                newVersionPolicy = server.createContentVersion(metadataContent.getContentId());
            } catch (CMException e2) {
                logger.log(Level.WARNING,
                           "While trying to fix referred Id in metadata " + referenceMetaDataId.getContentIdString()
                               + " supposed to point to " + referredContentId.getContentIdString() + ".");
            ...
            try {
                newVersion.setContentReference("polopoly.ReferenceMetaData", "referredId", referredContentId);
    ...
    private void setComponents(TextContent textContent, Content content) throws CMException {
    ...
    private void setReferences(TextContent textContent, Content content) throws CMException {
    ...
    private Policy createNewVersion(TextContent textContent) throws CMException, DeployException {
        ...
        if (contentId == null) {
            ...
            try {
                ...
                // create it again.
                logger.log(Level.WARNING, e.getMessage() + " Deleting it and recreating it.");
    ...
    private Policy createNewContent(TextContent textContent) throws DeployException, CMException {
        Policy newVersionPolicy;
    ...
    private Policy createNewVersionOfExistingContent(ContentId contentId) throws CMException,
        ExistingContentCouldNotBeReusedException {
        ...
        if (existingPolicyClassExists(content)) {
            ...
            throw new ExistingContentCouldNotBeReusedException(
                "Could not use the existing " + contentId.getContentId().getContentIdString()
    ...
    private boolean existingPolicyClassExists(ContentRead content) throws CMException {
        try {
            InputTemplate inputTemplate =
                (InputTemplate) cast(server.getContent(content.getInputTemplateId()), InputTemplate.class, content
                    .getInputTemplateId().getContentId().getContentIdString());
    ...
    private InputTemplate getInputTemplate(TextContent textContent) throws DeployException, CMException {
        ...
        try {
            ...
            throw new DeployException("The input template " + inputTemplateReference
                                      + " did not have InputTemplate as policy.");
        }
        return inputTemplate;
    }

    private Major getMajor(PolicyCMServer server, InputTemplate inputTemplate) throws CMException, DeployException {
        ...
        if (result == Major.UNKNOWN) {
            throw new DeployException("The input template " + inputTemplate.getName()
    ...
    public void setIgnoreContentListAddFailures(boolean ignoreContentListAddFailures) {
