...
public class TextContentSet implements Iterable<TextContent> {
	...
	public void sortTopologically() {
		Map<String, TextContentVertex> vertices = new HashMap<String, TextContentVertex>(
				contents.size());

		for (Map.Entry<String, TextContent> entry : contents.entrySet()) {
			vertices.put(entry.getKey(), new TextContentVertex(entry.getKey(),
					entry.getValue()));
		}
		...
		List<TextContentVertex> vertexList = new ArrayList<TextContentVertex>(
				vertices.values());
		TopologicalSorter<TextContentVertex> sorter = new TopologicalSorter<TextContentVertex>(
				vertexList);
	...
	private void addReferences(String id,
	...
	private void addListReferences(TextContent content,
	...
	private void addDirectReferences(TextContent content,
			TextContentVertex vertex,
			Map<String, TextContentVertex> knownVertices) {
	...
	private void addSecurityParentReference(TextContent content,
			TextContentVertex vertex,
			Map<String, TextContentVertex> knownVertices) {
		addDependencyToReference(vertex, content.getSecurityParent(),
				knownVertices);
	}

	private void addInputTemplateReference(TextContent content,
			TextContentVertex vertex,
			Map<String, TextContentVertex> knownVertices) {
		addDependencyToReference(vertex, content.getInputTemplate(),
				knownVertices);
	}

	private void addPublishingReference(TextContent content,
			TextContentVertex vertex,
			Map<String, TextContentVertex> knownVertices) {
		for (Publishing publishing : content.getPublishings()) {
			...
			String publishInId = ((ExternalIdReference) publishIn)
					.getExternalId();
	...
	private void addDependencyToReference(TextContentVertex vertex,
			Reference reference, Map<String, TextContentVertex> knownVertices) {
		if (!(reference instanceof ExternalIdReference)) {
			return;
		}

		ExternalIdReference externalIdReference = (ExternalIdReference) reference;
		TextContentVertex referredVertex = knownVertices
				.get(externalIdReference.getExternalId());
		if (referredVertex != null) {
			vertex.addDependency(referredVertex);
		}

		if (externalIdReference.getMetadataExternalId() != null) {
			referredVertex = knownVertices.get(externalIdReference
					.getMetadataExternalId());
			if (referredVertex != null) {
				vertex.addDependency(referredVertex);
			}
		}
