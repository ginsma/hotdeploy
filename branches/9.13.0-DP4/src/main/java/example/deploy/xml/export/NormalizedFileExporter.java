...
public class NormalizedFileExporter {
    ...
    public NormalizedFileExporter(PolicyCMServer server, DefaultContentContentsExporter contentsExporter,
    ...
    public void export(Set<ContentId> contentIdsToExport) {
        ...
        for (ContentId contentIdToExport : contentIdsToExport) {
            logger.log(Level.INFO, "Exporting " + count(contentIdsToExport, "object") + "...");
            ...
            try {
                ...
                logger.log(Level.WARNING, "While fetching content " +
                    contentIdToExport.getContentIdString() + " to export: " + e.getMessage(), e);
            }
        }
    ...
    private void exportSingleContent(ContentRead content) throws TransformerFactoryConfigurationError {
        File file = null;

        try {
            String externalId = externalIdGenerator.generateExternalId(content);

            String inputTemplate = server.getContent(content.getInputTemplateId()).getExternalId().getExternalId();

            file = namingStrategy.getFileName(
                    Major.getMajor(content.getContentId().getMajor()),
                    externalId, inputTemplate);
