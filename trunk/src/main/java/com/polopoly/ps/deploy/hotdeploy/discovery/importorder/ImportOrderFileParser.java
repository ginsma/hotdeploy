...
public class ImportOrderFileParser {
	private static final Logger logger = Logger
			.getLogger(ImportOrderFileParser.class.getName());
	...
	public ImportOrderFileParser(DeploymentDirectory directory,
	...
	public ImportOrderFileParser(File importOrderJavaIoFile) {
		this.importOrderFileAsDeploymentFile = new FileDeploymentFile(
				importOrderJavaIoFile);
		this.directory = new FileDeploymentDirectory(
				importOrderJavaIoFile.getParentFile());
		importOrderFile = new ImportOrderFile(directory);
	}
	...
	public ImportOrderFile parse() throws IOException {
		...
		logger.log(Level.FINE, "Reading import order from "
	...
	private void parseDependency(String line) {
		importOrderFile.addDependency(line
				.substring(DEPENDENCY_PREFIX.length()).trim());
	}
		...
		try {
			...
			if (NON_EXISTING_FILES.add(line)) {
				logger.log(Level.WARNING,
						"The directory or file specified as \"" + line
