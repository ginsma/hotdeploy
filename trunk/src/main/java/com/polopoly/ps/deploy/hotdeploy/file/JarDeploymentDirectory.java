...
public class JarDeploymentDirectory extends AbstractDeploymentObject implements
		DeploymentDirectory {
	private static final Charset UTF_8 = Charset.forName("UTF-8");
	private static final Charset LATIN_1 = Charset.forName("ISO-8859-1");

	private static final Logger LOGGER = Logger
			.getLogger(JarDeploymentDirectory.class.getName());
	...
	public DeploymentObject getFile(String fileName)
		...
		if (entry != null) {
			if (!entry.isDirectory()) {
				throw new RuntimeException("Attempt to get subdirectory "
						+ fileName + " from file " + this + ".");
			}
		...
		if (newEntry == null) {
			if (!isPureAscii(fileName)) {
				...
				throw new FileNotFoundException(fileName
						+ " could not be fetched from JAR " + jarFile.getName());
			}
		}
	...
	private ZipEntry getEntryUsingEncoding(String fileName, Charset charset)
			throws FileNotFoundException {
		String platformEncodedFileName = new String(fileName.getBytes(charset),
				UTF_8);

		ZipEntry newEntry = jarFile.getEntry(platformEncodedFileName);

		System.out.println(platformEncodedFileName + " " + newEntry + " "
				+ charset.displayName());
	...
	public String getName() {
		...
		if (nameWithinJar.endsWith("/")) {
			nameWithinJar = nameWithinJar.substring(0,
					nameWithinJar.length() - 1);
		}
	...
	public String getRelativeName(DeploymentObject deploymentObject) {
		if (deploymentObject instanceof JarDeploymentFile
				&& ((JarDeploymentFile) deploymentObject).getJarFile().equals(
						getJarFile())) {
			String fileName = ((JarDeploymentFile) deploymentObject)
					.getNameWithinJar();
	...
	public boolean imports(DeploymentObject object) {
		if (object instanceof JarDeploymentDirectory) {
			return ((JarDeploymentDirectory) object).getJarFile().equals(
					jarFile)
					&& ((JarDeploymentDirectory) object).getNameWithinJar()
							.startsWith(getNameWithinJar());
		}

		if (object instanceof JarDeploymentFile) {
			return ((JarDeploymentFile) object).getJarFile().equals(jarFile)
					&& ((JarDeploymentFile) object).getNameWithinJar()
							.startsWith(getNameWithinJar());
		}
