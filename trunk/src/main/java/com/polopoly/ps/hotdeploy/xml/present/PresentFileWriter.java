...
public class PresentFileWriter {
    private static final Logger LOGGER = Logger
            .getLogger(PresentFileWriter.class.getName());
    ...
    private void writeTemplate(final AllContent allContent) {
        ...
        try {
            templateFile = new File(rootDirectory,
                    PresentFileReader.PRESENT_TEMPLATES_FILE);
            FileWriter templateWriter = new FileWriter(templateFile);

            for (String externalId : allContent
                    .getExternalIds(Major.INPUT_TEMPLATE)) {
            ...
            LOGGER.log(Level.WARNING, "Could not write to "
                    + (templateFile != null ? templateFile.getAbsoluteFile()
    ...
    private void writeContent(final AllContent allContent) {
        ...
        try {
            contentFile = new File(rootDirectory,
                    PresentFileReader.PRESENT_CONTENT_FILE);
            ...
            LOGGER.log(Level.WARNING, "Could not write to "
                    + (contentFile != null ? contentFile.getAbsoluteFile()
                            : rootDirectory) + ": " + e.getMessage(), e);
        }
    }
}
