...
import com.polopoly.ps.hotdeploy.state.FileListPseudoPolicy.NoFileAtIndexException;
...
public class FileChecksumsPseudoPolicy {
	...
	private LongComponent getQuickChecksumComponent(DeploymentFile file) {
		LongComponent result = new LongComponent(getAttributeGroup(file),
	...
	public void deleteChecksums(DeploymentFile file)
		...
		try {
			getQuickChecksumComponent(file)
					.setStringValue(delegatePolicy, null);
			getSlowChecksumComponent(file).setStringValue(delegatePolicy, null);

			FileListPseudoPolicy fileList = new FileListPseudoPolicy(
					delegatePolicy);
			...
			throw new CouldNotUpdateStateException(
					"While deleting stored state for " + file + ": " + e, e);
		}
	}
	...
	public void clear() {
		try {
			...
			logger.log(Level.WARNING,
					"While clearing file checksums: " + e.getMessage(), e);
		}
	}
