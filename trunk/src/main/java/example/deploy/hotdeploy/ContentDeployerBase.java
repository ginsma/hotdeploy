...
public class ContentDeployerBase
{
    ...
    public ContentDeployerBase(PolicyCMServer server) {
        this(server, false);
    }

    public ContentDeployerBase(PolicyCMServer server, boolean failFast) {
                                           ...
                                           DirectoryState strategy)
    {
        ...
        logger.log(Level.FINER, "Found " + files.length + " XML files in " + directory.getAbsolutePath() + ". Checking for modified files.");
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (strategy.hasFileChanged(file)) {
                ...
                logger.log(Level.FINEST, file.getName() + " had been modified.");

                anyFileChanged = true;
            }
        }

        logger.log(Level.FINER, result.size() + " file(s) had been modified since last import.");
