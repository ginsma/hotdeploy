...
public class BootstrapFileGenerator {
    ...
    public void generateBootstrap(File directory, List<DeploymentFile> deploymentFiles) {
        System.err.println(deploymentFiles.size() +  " content XML file" + plural(deploymentFiles) + " to generate bootstrap for.");

        File bootstrapFile = getBootstrapFile(directory);
        DeploymentFile bootstrapFileAsDeploymentFile = new FileDeploymentFile(bootstrapFile);
        ...
        Bootstrap bootstrap =
            new BootstrapGenerator(parser).generateBootstrap(deploymentFiles);
        ...
        if (!bootstrap.isEmpty()) {
            ...
            new BootstrapToImportOrderAdder(bootstrapFile).addBootstrapToImportOrderIfItExists();
        }
    }
